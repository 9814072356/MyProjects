...
public class Guest_preGame {
	...
	public void set(Stage stage) {
		...
		LnN = new Group();
		LnN = BoardUI.drawUI(boardUI.getTranslateX(),boardUI.getTranslateY(),BOARD_DIM,25);
		...
		message.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if(counter == SHIP_QUANTITY_5*5+SHIP_QUANTITY_4*4+SHIP_QUANTITY_3*3+SHIP_QUANTITY_2*2||true) {
					...
					Scene scene7 = new Scene(guestUI.getRoot(), screenWidth, screenHeight);
					guestUI.init(scene7,stage);
					scene7.setFill(cycleGrad);
					guestUI.sendShipsToServer(shipString);
					for(int row = 0; row < BOARD_DIM; row++) {
						for(int col = 0; col < BOARD_DIM; col++) {
							if(board.getCell(row, col) == BoardTag.BRIG) {
								guestUI.getenemyBoard().setCell(row, col, BoardTag.BRIG);
								count++;
								((Rectangle)guestUI.getenemyBoardUI().getChildren().get(row*BOARD_DIM+col)).setFill(Color.web("#EC7063"));
							}else {
								guestUI.getenemyBoard().setCell(row, col, BoardTag.EMPTY);
								((Rectangle)guestUI.getenemyBoardUI().getChildren().get(row*BOARD_DIM+col)).setFill(Color.LIGHTSKYBLUE);
							}
						}
					}
					guestUI.setShipBlocksOnBoardE(count);
